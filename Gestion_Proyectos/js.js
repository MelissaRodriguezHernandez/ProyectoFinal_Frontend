 ...
 function enviarRegistro() {
     ...
     if(nombre.length > 15 || nombre.length == 0){
         alert("El nombre es incorrecto o falta");
     }else{
         if(apellidos.length > 40 || apellidos.length == 0){
             alert("Los apellidos son incorrectos o faltantes")
         }else{
             if(email.length <10 || email.length>40 || email.length == 0){
                alert("Error en la estructura del email");
             }
             else{
                 if(clave.length < 9 || clave.length > 40 || clave == 0){
